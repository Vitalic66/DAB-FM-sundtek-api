...
void SuperframeFilter::Feed(const uint8_t *data, size_t len) {
    qDebug() << "SuperframeFilter::Feed in dabplus_decoder.cpp 49--------------------------------------------->" << "start";
	// check frame len
	if(frame_len) {
		if(frame_len != len) {
...
void SuperframeFilter::CheckForPAD(const uint8_t *data, size_t len) {
    //qDebug() << "SuperframeFilter::CheckForPAD dabplus_decoder.cpp 145 started";
    qDebug() << "SuperframeFilter::CheckForPAD in dabplus_decoder.cpp 145--------------------------------------------->" << "start";

    qDebug() << "dabplus_decoder raw-data in *data" << *data;
    qDebug() << "dabplus_decoder raw-data in data" << data;
    qDebug() << "dabplus_decoder raw-data in len" << len;
    //qDebug() << "data[0]" << data[0];
    //qDebug() << "data[0] >> 5" << (data[0] >> 5);

    //qDebug() << "data[0] " << data[0] << " data[1] " << data[1] << " data[2] " << data[2];
	bool present = false;
	...
	if(len >= 3 && (data[0] >> 5) == 4) {
        ...
        //qDebug() << "pad_len = data[1] " << pad_len << " data[1] " << data[1];
		if(pad_len == 255) {
			pad_len += data[2];
            //qDebug() << "pad_len += data[2] " << pad_len << " data[2] " << data[2];
			pad_start++;
            //qDebug() << "nach pad_start++" << pad_start;
		}
        //dab_decoder:
		...
		if(pad_len >= 2 && len >= pad_start + pad_len) {
            ...
            qDebug() << "dabplus_decoder calc data " << data << " pad_start " << pad_start;
            qDebug() << "dabplus_decoder calc data + pad_start" << data + pad_start;
            qDebug() << "dabplus_decoder calc pad_len " << pad_len << " FPAD_LEN " << FPAD_LEN;
            qDebug() << "dabplus_decoder calc pad_len - FPAD_LEN" << pad_len - FPAD_LEN;
            qDebug() << "dabplus_decoder calc data + pad_start + pad_len - FPAD_LEN" << data + pad_start + pad_len - FPAD_LEN;
